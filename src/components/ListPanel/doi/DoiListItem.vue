	...
	<div :id="`list-item-${item.type}-${item.id}`" class="listPanel__item--doi">
		<div class="listPanel__itemSummary">
			...
			<div class="listPanel__itemActions">
				...
				<div class="doiListItem__itemMetadata">
					<Badge
						:is-primary="item.isPublished && isDeposited(item.depositStatus)"
						:is-warnable="
							item.isPublished &&
							((!isDeposited(item.depositStatus) &&
								isRegistrationPluginConfigured) ||
								(isDeposited(item.depositStatus) &&
									isRegistrationPluginConfigured &&
									isStale(itemDepositStatus)) ||
								hasErrors(item.depositStatus))
						"
		...
		>
			...
			>
				...
				<PkpButton
					:is-disabled="isDeposited(item.depositStatus) || isSaving"
			...
			>
				...
				<div class="doiListItem__depositorActions">
					<PkpButton
						v-if="isDeposited(item.depositStatus) && hasRegisteredMessage"
					...
					<PkpButton
						v-else-if="!isDeposited(item.depositStatus) && item.isPublished"
						:is-disabled="isEditingDois"
					...
					<PkpButton
						v-if="hasErrors(item.depositStatus) && hasErrorMessage"
						ref="errorMessageModalButton"
						:is-disabled="isEditingDois"
						@click="
							openViewErrorModal(currentVersionDoiObjects[0]['errorMessage'])
						"
					>
...
export default {
	...
	data() {
		...
		return {
			doiListColumns: [
				...
				{
					name: 'actions',
					label: this.t('grid.columns.actions'),
					value: 'value',
				},
			],
			isEditingDois: false,
	...
	methods: {
		...
		handleDepositorActions() {
			return this.isDeposited(this.item.depositStatus)
				? this.viewRecord()
				: this.triggerDeposit();
		},
		viewRecord() {
